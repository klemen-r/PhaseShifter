...
pub fn calculate_clusters(
    ...
    // Step 2: Convert nodes to deviation space
    ...
    // Step 3: Separate by side and cluster each
    let bullish_nodes: Vec<_> = cluster_nodes
        .iter()
        .filter(|n| n.original.side == ProjectionSide::Bullish)
        .cloned()
        .collect();

    let bearish_nodes: Vec<_> = cluster_nodes
        .iter()
        .filter(|n| n.original.side == ProjectionSide::Bearish)
        .cloned()
        .collect();
    ...
    if bullish_nodes.len() >= 2 {
        let bullish_clusters =
            cluster_side(&bullish_nodes, ProjectionSide::Bullish, anchor, config);
        clusters.extend(bullish_clusters);
    }

    // Cluster bearish nodes
    if bearish_nodes.len() >= 2 {
        let bearish_clusters =
            cluster_side(&bearish_nodes, ProjectionSide::Bearish, anchor, config);
...
fn compute_anchor(nodes: &[ProjectionNode]) -> Option<f64> {
    let derived_anchors: Vec<f64> = nodes
        .iter()
        .filter_map(|n| {
            if n.distance_pct.abs() < 1e-10 {
                return None; // Skip zero-distance nodes
            }
...
fn cluster_side(
    ...
    sorted_nodes.sort_by(|a, b| a.deviation.partial_cmp(&b.deviation).unwrap());

    // Calculate gaps between consecutive nodes
    let gaps: Vec<f64> = sorted_nodes
        .windows(2)
    ...
    let seed_groups = initial_seeding(&sorted_nodes, seed_gap);
    ...
    for group in seed_groups {
        ...
        let center = median(&group.iter().map(|n| n.deviation).collect::<Vec<_>>());
        ...
        for subcluster in subclusters {
            ...
            let deviations: Vec<f64> = subcluster.iter().map(|n| n.deviation).collect();
            ...
            let unique_scenarios: HashSet<_> =
                subcluster.iter().map(|n| n.scenario.clone()).collect();
...
fn initial_seeding(sorted_nodes: &[ClusterNode], seed_gap: f64) -> Vec<Vec<ClusterNode>> {
    let mut groups = Vec::new();
    let mut current_group = vec![sorted_nodes[0].clone()];

    for i in 1..sorted_nodes.len() {
        let gap = (sorted_nodes[i].deviation - sorted_nodes[i - 1].deviation).abs();
        if gap <= seed_gap {
            current_group.push(sorted_nodes[i].clone());
        } else {
            if !current_group.is_empty() {
                groups.push(current_group);
            }
            current_group = vec![sorted_nodes[i].clone()];
        }
    }
...
fn subcluster(nodes: &[ClusterNode], core_gap: f64) -> Vec<Vec<ClusterNode>> {
    if nodes.is_empty() {
        return Vec::new();
    }
...
impl ClusterManager {
    ...
    pub fn get_clusters(&mut self, symbol: &str) -> Option<ClustersData> {
        ...
        if let Some(cached) = self.cache.get(symbol) {
            debug!("Returning cached clusters for {}", symbol);
            return Some(cached.clone());
        }

        let tracked_nodes = self.nodes.get(symbol)?;

        // Filter to only OPEN nodes
        let open_nodes: Vec<_> = tracked_nodes
            .iter()
            .filter(|n| n.status == NodeStatus::Open)
            .collect();
        ...
        if open_nodes.is_empty() {
            info!("No open nodes for {}, returning None", symbol);
            return None;
        }

        // Convert to ProjectionNodes, each using its own scenario's anchor
        let mut projection_nodes: Vec<ProjectionNode> = Vec::new();
        ...
        if projection_nodes.is_empty() {
            info!("No projectable nodes for {} (missing anchors?)", symbol);
            return None;
        }
        ...
        let node_infos: Vec<NodeInfo> = projection_nodes
            .iter()
            .map(|n| NodeInfo {
                value: n.value,
                side: n.side,
        ...
        // Cache result
        self.cache.insert(symbol.to_string(), result.clone());
